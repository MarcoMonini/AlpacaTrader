...
def pivots_on(fig, df, piv) -> None:
    """Hollow squares on the price, red for highs and green for lows.

    Hollow and small on purpose: the oracle marks every pivot in the window, so at equal weight the
    squares are what the eye reads first and the candles disappear behind them.
    """
...
def chart(df, symbol: str, timeframe: str, piv=None) -> go.Figure:
...
def main() -> None:
    ...
    st.plotly_chart(chart(df, symbol, timeframe, piv), use_container_width=True)
    ...
    if took is not None:
        ...
        columns[1].metric(
            ...
            help="summed over every leg, before fees — the sum of the moves, not a compounded return",
        )
        columns[2].metric(
            "Net total",
        ...
        columns[5].metric("Median bars", f"{took.bars.median():.0f}" if len(took) else "—")
